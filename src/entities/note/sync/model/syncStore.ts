...
export const useSyncStore = create<SyncState>((set) => ({
  ...
  markProcessing: (opId) =>
    set((state) => ({
      queue: state.queue.map((op) =>
        op.opId === opId ? { ...op, status: "processing" } : op
      ),
    })),
  ...
  markError: (opId, error) =>
    set((state) => ({
      queue: state.queue.map((op) =>
        op.opId === opId ? { ...op, status: "error", lastError: error } : op
      ),
    })),
  ...
  incrementRetry: (opId) =>
    set((state) => ({
      queue: state.queue.map((op) =>
        op.opId === opId ? { ...op, retryCount: op.retryCount + 1 } : op
      ),
    })),
